import $hgUW1$dailycodailyjs from "@daily-co/daily-js";
import {RTVIError as $hgUW1$RTVIError, TransportStartError as $hgUW1$TransportStartError, RTVIMessage as $hgUW1$RTVIMessage, Transport as $hgUW1$Transport} from "realtime-ai";
...
class $845232e2f6d6caea$export$b1ca982aa1e488c1 extends (0, $hgUW1$Transport) {
    constructor(){
        super();
        this._botId = "";
        this._selectedCam = {};
    ...
    initialize(options, messageHandler) {
        ...
        const existingInstance = (0, $hgUW1$dailycodailyjs).getCallInstance();
        if (existingInstance) existingInstance.destroy();
        this._daily = (0, $hgUW1$dailycodailyjs).createCallObject({
    ...
    async initDevices() {
        if (!this._daily) throw new (0, $hgUW1$RTVIError)("Transport instance not initialized");
    ...
    async connect(authBundle, abortController) {
        ...
        try {
            ...
            this.state = "error";
            throw new (0, $hgUW1$TransportStartError)();
        }
        if (abortController.signal.aborted) return;
            ...
            (async ()=>{
                this._daily.on("track-started", (ev)=>{
                    if (!ev.participant?.local) {
                        this.state = "ready";
                        this.sendMessage((0, $hgUW1$RTVIMessage).clientReady());
                        resolve();
                    }
                });
            })();
    ...
    handleTrackStarted(ev) {
        this._callbacks.onTrackStarted?.(ev.track, ev.participant ? $845232e2f6d6caea$var$dailyParticipantToParticipant(ev.participant) : undefined);
    }
    handleTrackStopped(ev) {
        this._callbacks.onTrackStopped?.(ev.track, ev.participant ? $845232e2f6d6caea$var$dailyParticipantToParticipant(ev.participant) : undefined);
    }
    handleParticipantJoined(ev) {
        const p = $845232e2f6d6caea$var$dailyParticipantToParticipant(ev.participant);
    ...
    handleRemoteAudioLevel(ev) {
        ...
        for(let i = 0; i < ids.length; i++){
            ...
            this._callbacks.onRemoteAudioLevel?.(level, $845232e2f6d6caea$var$dailyParticipantToParticipant(participants[id]));
        }
    }
    handleLeftMeeting() {
...
const $845232e2f6d6caea$var$dailyParticipantToParticipant = (p)=>({
        id: p.user_id,
        local: p.local,
        name: p.user_name
    });
...
export {$845232e2f6d6caea$export$b1ca982aa1e488c1 as DailyTransport};
